...
public final class Worker implements CSProcess {
    ...
    Ask.app (
				"Ray Tracer",
				"Implements a basic ray-tracing algorithm incorporating reflection, \nrefraction and texture mapped " +
				"surfaces, distributed using a farmer/worker/harvester approach. \nThis is a worker node. By specifying " +
				...
				"\nTo leave the network, press ENTER. \nDo not terminate the program with CTRL+C or the worker may not " +
				"deregister itself with the farmer and cause a deadlock.");
    serverAddress = Ask.string ("Server address? : ");
		mainNodeAddress = new TCPIPNodeAddress(serverAddress, 3000);
    System.out.println ("Server running on " + mainNodeAddress.getIpAddress());
      ...
      workerNodeAddress = new TCPIPNodeAddress(workerAddress, 3000);
