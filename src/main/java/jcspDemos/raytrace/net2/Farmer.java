...
final class Farmer implements CSProcess {
	...
	private final void workerLeaves (NetChannelLocation ncl) {
		...
		int j = 0;
		for (int i = 0; i < numWorkers; i++) {
      String workerStr = ((NetChannelOutput)toWorkers[i]).getLocation ().toString();
      String leaverStr = ncl.toString();
			if (!workerStr.equals (leaverStr)) {
	...
	private final void workerJoins (NetChannelLocation ncl) {
		...
		toWorkers2[numWorkers] = NetChannel.one2net (ncl);
